...
extern char* format_overwrite(format_t* f, const char* format, ...) __attribute__((format(printf, 2, 3)));
extern char* strf(char* buf, int64_t size, const char *f, ...) __attribute__((format(printf, 3, 4))) ;
extern __thread format_t g_format;
#define T2S(type, obj) type ## _str(&g_format, obj)
